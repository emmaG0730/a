...
class AssetBrowser(QtGui.QDialog):	
	...
	def open_corresponding_file(self):		
		...
		file_path = path[0]
		file_namespace =  path[1]
		current_sence = mc.file(q = True, sceneName = True)			
		if mc.file(q = True,modified = True):
			result = QtGui.QMessageBox.question(self,'Modified',
			'Save changes to ' + str(current_sence) + '?',QtGui.QMessageBox.StandardButton.Yes, QtGui.QMessageBox.StandardButton.No)
			print 'result : ' + str(result)
			if result == QtGui.QMessageBox.StandardButton.Yes:
				mc.file(type='mayaAscii', force = True, save = True)				
	...
	def contextMenuEvent(self, event):
		...
		context_menu.addAction((QtGui.QAction("Open", self,triggered = self.open_corresponding_file)))
		context_menu.addAction((QtGui.QAction("Import", self,triggered = self.import_corresponding_file)))
		context_menu.addAction((QtGui.QAction("Reference", self,triggered = self.reference_corresponding_file)))
